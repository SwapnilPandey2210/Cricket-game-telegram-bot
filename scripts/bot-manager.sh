...
kill_all_bot_processes() {
    log_message "Stopping all bot processes..."
...
case "$1" in
    start)
        log_message "Starting bot manager..."
        kill_all_bot_processes
        sleep 3
        start_bot
        if [ "$?" -eq 0 ]; then
            log_message "Bot manager started successfully"
            monitor_bot &
        ...
        ;;
    stop)
        log_message "Stopping bot manager..."
        ...
        kill_all_bot_processes
        log_message "Bot manager stopped"
        ;;
    restart)
        log_message "Restarting bot manager..."
        $0 stop
        sleep 2
        $0 start
        ;;
