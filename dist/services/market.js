export async function listForSale(prisma, userId, cardId, quantity, price) {
    ...
    if (!own || own.quantity < quantity)
        throw new Error('Not enough quantity to list.');
    ...
    if (existing)
        throw new Error('You already have an active listing for this card at this price.');
...
export async function browseMarket(prisma, limit = 20) {
    return prisma.listing.findMany({ where: { active: true }, include: { card: true, seller: true }, take: limit, orderBy: { createdAt: 'desc' } });
}
export async function buyFromMarket(prisma, buyerId, listingId, quantity) {
    if (quantity <= 0)
    ...
    return prisma.$transaction(async (tx) => {
        ...
        return { ok: true, spent: total, listingId: listing.id };
    });
}
export async function cancelListing(prisma, userId, listingId) {
    return prisma.$transaction(async (tx) => {
        const listing = await tx.listing.findUnique({ where: { id: listingId } });
        if (!listing || listing.sellerId !== userId)
            throw new Error('Listing not found or not yours.');
        if (!listing.active)
            return { ok: false, message: 'Already inactive.' };
